...
function triggerLSRAction(xmpp, routingTable) {
    ...
    const { bestNeighbor, totalCost } = findBestNeighbor(from, destination, routingTable);

    if (bestNeighbor) {
        console.log(`Sending LSR message to ${destination} via ${bestNeighbor} with total cost ${totalCost}`);
...
function handleLSRMessage(stanza, xmpp, routingTable) {
    ...
    console.log(`LSR message received from ${message.from} with payload: "${message.payload}"`);

    if (message.destination === currentJid) {
        console.log(`Message received at final destination: ${message.payload}`);
    } else {
        const { bestNeighbor, totalCost } = findBestNeighbor(currentJid, message.destination, routingTable);
        
        if (bestNeighbor) {
            console.log(`Forwarding LSR message to ${message.destination} via ${bestNeighbor} with remaining cost ${totalCost}`);
            sendLSRMessage(xmpp, currentJid, bestNeighbor, message.payload, routingTable[currentJid], message.destination);
        } else {
